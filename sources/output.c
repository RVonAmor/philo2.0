...
/* write_status_normal:
*	Handles the normal (non-debug) output of philosopher status messages.
*	Uses print_status with the appropriate message for each status type.
*/
static void	write_status_normal(t_philo *philo, t_status status)
{
	if (status == DIED)
...
*	subject.
*
*	Uses a modular approach by delegating to appropriate status writer
*	functions based on debug mode.
*/
void	write_status(t_philo *philo, bool reaper_report, t_status status)
{
	...
	if (DEBUG_FORMATTING == true)
		...
		write_status_normal(philo, status);
...
*	Uses mutex protection when reading each philosopher's meal count
...
void	write_outcome(t_table *table)
{
	...
	while (i < table->nb_philos)
	{
		...
		times_ate = table->philos[i]->times_ate;
		pthread_mutex_unlock(&table->philos[i]->meal_time_lock);
		
		if (times_ate >= (unsigned int)table->must_eat_count)
			full_count++;
		i++;
	}
